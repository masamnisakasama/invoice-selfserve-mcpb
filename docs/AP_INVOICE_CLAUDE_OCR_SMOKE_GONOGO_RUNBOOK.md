...
This package is currently at the vNext Milestone 0 gate. The packaged MCP server intentionally blocks legacy AP review tools so the demo cannot fall back to sidecar JSON while the Claude OCR path is still unverified.
...
8. Do not continue to AP folder review with this package. After `OCR_SMOKE_TEST_PASSED`, the next implementation step is the no-sidecar Claude OCR review flow.
...
- Q7a. Legacy sidecar review MCP tools are intentionally blocked in the distributed Claude OCR gate package.
...
このパッケージは現時点では vNext Milestone 0 のGo/No-Go確認段階です。Claude OCR経路が未確認のままsidecar JSONへfallbackしないよう、配布MCP serverでは旧APレビューtoolを意図的にブロックしています。
...
8. このパッケージでAPフォルダレビューへは進めないでください。`OCR_SMOKE_TEST_PASSED` 後の次ステップは、no-sidecar Claude OCRレビュー本体の実装です。
...
- Q7a. 配布Claude OCR gate packageでは、旧sidecarレビューMCP toolsを意図的にブロックしています。
